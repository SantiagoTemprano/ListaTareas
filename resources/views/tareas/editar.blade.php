            ...
            <div class="bg-white p-6 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg max-w-2xl">
                <form action="{{ route('tareas.update',$tarea) }}" method="post">
                    ...
                    @error('texto')
                        <div class="text-sm mt-1 text-red-500">{{ $message }}</div>
                    @enderror
                    <select name="listaTareas_id" class="w-full mt-6 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                        <option value="">-- Selecciona una lista --</option>
                        @foreach ($listaTareas as $listaTarea)
                            <option value="{{ $listaTarea->id }}" @if ($tarea->listaTareas_id == $listaTarea->id ) selected  @endif>{{ $listaTarea->nombre }}</option>
                        @endforeach
                    </select>
