    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white p-6 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg max-w-2xl">
                <form action="{{ route('listaTareas.update',$listaTareas) }}" method="post">
                    @method('put')
                    @csrf
                    <x-text-input name="nombre" class="w-full" placeholder="Nombre" value="{{ @old('nombre',$listaTareas->nombre) }}"></x-text-input>
                    @error('nombre')
